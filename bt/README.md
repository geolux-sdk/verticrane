...
**이 링크에는 페어링도 암호화도 없다.** 캐릭터리스틱이
`["write", "write-without-response"]` 로 열려 있어, 범위 안의 누구나 붙어서 명령을
보낼 수 있다. 두 가지가 따라온다.

- **비밀번호 노출** — `{"cmd":"secret"}` 가 저장된 PSK 를 평문으로 돌려주고,
  `connect` 로 보내는 PSK 도 스니핑될 수 있다.
- **장비를 못 쓰게 만들 수 있다** — `connect` 로 **존재하지 않는 SSID 로 옮겨버리면**
  화면도 키보드도 없는 크레인 위 장비에서 데이터를 가져올 방법이 사라진다. 되돌릴
  유일한 경로가 방금 그 공격에 쓰인 BLE 다. 이쪽이 더 무겁다 — 차량 Wi-Fi 접근
  권한이 **전혀 없는** 사람이 측정 전체를 무산시킬 수 있다.

### 광고 창

그래서 **광고는 부팅 후 정해진 시간만 열린다**(기본 180초,
`VERTICRANE_BLE_WINDOW_S`). 창이 닫히면 아무도 장비를 찾지 못하고, **다시 열려면
전원을 껐다 켜야 한다.**

이 문서는 한때 "설정 시점에만 쓰는 채널이라 위험 노출 시간은 짧다"고 적었는데,
서비스가 systemd 로 상시 기동되어 사실이 아니었다. 창이 그 문장을 사실로 만든다.

**설정 중에 창이 닫히지는 않는다.** 호스트가 붙어 있는 동안은 그대로 두고, 끊고
나간 뒤에 내린다.

더 강하게 가려면 다음 중 하나가 필요하다.

- 캐릭터리스틱에 `encrypt-write` + LE Secure Connections 본딩 요구. 다만 Just Works
  본딩은 공격자도 페어링하면 그만이라 **비밀번호 노출만 막고 장비 무력화는 못 막는다.**
  제대로 막으려면 **페어링 패스키를 e-paper 에 표시**해야 한다 — 화면을 볼 수 있는
  사람은 이미 장비 앞에 있는 사람이다.
