...
지금 무엇을 하는 중이냐에 따라 **세 가지 화면**이 나옵니다.

**설치 중** — 전원을 켠 직후부터, 기록이 시작되고 1분까지
...
- 맨 아래 **SSID가 반전(검은 띠)이면 그 네트워크에 붙어 있는 것**입니다.
  평범한 글씨면 아직 안 붙은 것이고, 위에 `NO NETWORK`가 함께 나옵니다.
  크레인 위에서는 정상이며 기록에는 지장이 없습니다.
- **시각은 표시하지 않습니다.** 틀린 시각은 없느니만 못하고, 믿을 수 있는 시각은
  파일 이름에 있습니다.
